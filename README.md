...
> WebSocket 기반 실시간 채팅 중고거래 플랫폼 — 백엔드
...
JWT 인증, Google 소셜 로그인, 이메일/SMS 본인인증, 아임포트 기반 자체 페이 시스템,
관리자 대시보드까지 실서비스 수준의 기능을 갖추었습니다.
...
|------|------|
| 로그인 / 회원가입 / 인증 API | 송은설 |
| 소셜 로그인 (Google OAuth2) | 송은설 |
| 이메일 / SMS 인증 | 송은설 |
| 결제 (아임포트 연동) | 김민석 |
| 채팅 (WebSocket, Redis Pub/Sub) | 강태준 |
| 알림 API | 강태준 |
| Spring Batch 채팅 로그 저장 | 강태준 |
| 상품 CRUD / 찜 / 이미지 업로드 | 강석영 |
| 신고 기능 | 강석영 |
| 관리자 대시보드 / 통계 API | 김태헌 |
| 관리자 회원·상품·카테고리 관리 API | 김태헌 |
| 공지사항 / 자주묻는질문 API | 김덕식 |
...
|------|------|
| GCP 서버 구성 | 강태준 |
| CI/CD 자동화 (GitHub Actions) | 강태준 |
| Docker Compose 구성 | 강태준 |
